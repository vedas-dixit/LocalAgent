...
def test_save_md_plus_ignores_cwd_and_writes_to_project_root(tmp_path):
    original_cwd = os.getcwd()
    try:
        os.chdir(tmp_path)
        res = save_md_plus.run("Segment from CWD test", "cwd_plus_test")
        assert isinstance(res, str)
        m = re.search(r"Markdown segment appended: (.*\.md)$", res)
